...
# 颜色配置
COLOR_TITLE = (0, 212, 255)
COLOR_SUBTITLE = (255, 215, 0)
COLOR_TEXT = (224, 224, 224)
COLOR_PHASE_BG = (30, 50, 60, 180)
COLOR_WARNING = (255, 107, 107)
...
def draw_phase_card(draw, y_offset, phase):
    ...
    draw_rounded_rectangle(draw, [card_x1, card_y1, card_x2, card_y2], radius=15, fill=COLOR_PHASE_BG)
    ...
    draw.text((circle_x + circle_radius + 20, card_y1 + 25), phase["title"], fill=title_color, font=title_font)
    
    time_font = get_font(22)
    draw.text((circle_x + circle_radius + 20, card_y1 + 75), phase["time"], fill=(150, 150, 150), font=time_font)
    ...
    for item in phase["items"]:
        check_x = card_x1 + 90
        draw.text((check_x, item_y), "✓", fill=COLOR_ACCENT, font=get_font(28, bold=True))
        draw.text((check_x + 35, item_y + 2), item, fill=COLOR_TEXT, font=item_font)
        item_y += 45
...
def generate_poker_checklist():
    ...
    draw.text(((WIDTH - title_width) // 2, y_pos), title_text, fill=COLOR_TITLE, font=title_font)
    ...
    draw.text(((WIDTH - subtitle_width) // 2, y_pos), subtitle_text, fill=COLOR_SUBTITLE, font=subtitle_font)
    
    y_pos += 70
    draw.rectangle([PADDING * 2, y_pos, WIDTH - PADDING * 2, y_pos + 3], fill=COLOR_TITLE)
    y_pos += 40
    ...
    draw_rounded_rectangle(draw, [PADDING * 2, tips_y1, WIDTH - PADDING * 2, tips_y2], radius=15, fill=(255, 215, 0, 40))
    
    draw.rectangle([PADDING * 2, tips_y1, PADDING * 2 + 3, tips_y2], fill=COLOR_SUBTITLE)
    draw.rectangle([WIDTH - PADDING * 2 - 3, tips_y1, WIDTH - PADDING * 2, tips_y2], fill=COLOR_SUBTITLE)
    draw.rectangle([PADDING * 2, tips_y1, WIDTH - PADDING * 2, tips_y1 + 3], fill=COLOR_SUBTITLE)
    draw.rectangle([PADDING * 2, tips_y2 - 3, WIDTH - PADDING * 2, tips_y2], fill=COLOR_SUBTITLE)
    
    tips_font = get_font(30, bold=True)
    tips_text = "💡 核心原则"
    bbox = draw.textbbox((0, 0), tips_text, font=tips_font)
    tips_width = bbox[2] - bbox[0]
    draw.text(((WIDTH - tips_width) // 2, tips_y1 + 25), tips_text, fill=COLOR_SUBTITLE, font=tips_font)
    
    content_font = get_font(22)
    content_text1 = "保持纪律 · 理性决策 · 不被情绪左右"
    bbox = draw.textbbox((0, 0), content_text1, font=content_font)
    content_width = bbox[2] - bbox[0]
    draw.text(((WIDTH - content_width) // 2, tips_y1 + 75), content_text1, fill=COLOR_TEXT, font=content_font)
    
    content_text2 = "每个决策点都走完流程 · 形成肌肉记忆"
    bbox = draw.textbbox((0, 0), content_text2, font=content_font)
    content_width = bbox[2] - bbox[0]
    draw.text(((WIDTH - content_width) // 2, tips_y1 + 110), content_text2, fill=COLOR_TEXT, font=content_font)
    ...
    draw.text(((WIDTH - footer_width) // 2, footer_y), footer_text, fill=COLOR_SUBTITLE, font=footer_font)
    ...
    draw.text(((WIDTH - subfooter_width) // 2, footer_y + 45), subfooter_text, fill=(150, 150, 150), font=subfooter_font)
...
if __name__ == "__main__":
    try:
        output_file = generate_poker_checklist()
        print(f"\n✨ 图片已保存到桌面: {output_file}")
        print("\n🎉 完成！请到桌面查看图片文件。")
