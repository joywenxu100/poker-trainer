// 实战行动指南 V4.0 - 完整街道覆盖版
// 核心功能：分街道快速场景 + 自定义输入 + 全屏警告 + 个人错误提醒

let state = {
    position: 'ip',
    street: 'turn',
    hand: 'top-two',
    action: 'check',
    villain: 'unknown',
    danger: 'dangerous'
};

// 街道切换
function switchStreet(street) {
    document.querySelectorAll('.street-tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.scenario-panel').forEach(p => p.classList.remove('active'));
    
    document.querySelector(`.street-tab[onclick="switchStreet('${street}')"]`).classList.add('active');
    document.getElementById(`panel-${street}`).classList.add('active');
    
    // 隐藏结果和自定义模式
    document.getElementById('resultSection').classList.remove('show');
    document.getElementById('customMode').classList.remove('show');
}

// 返回场景列表
function goBack() {
    document.getElementById('resultSection').classList.remove('show');
    document.getElementById('fullscreenWarning').classList.remove('show');
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

// 初始化按钮事件
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('[data-position],[data-street],[data-hand],[data-action],[data-villain],[data-danger]').forEach(btn => {
        btn.addEventListener('click', function() {
            const group = btn.closest('.btn-grid');
            group.querySelectorAll('.btn').forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
            
            if(btn.dataset.position) state.position = btn.dataset.position;
            if(btn.dataset.street) state.street = btn.dataset.street;
            if(btn.dataset.hand) state.hand = btn.dataset.hand;
            if(btn.dataset.action) state.action = btn.dataset.action;
            if(btn.dataset.villain) state.villain = btn.dataset.villain;
            if(btn.dataset.danger) state.danger = btn.dataset.danger;
        });
    });
});

// 切换自定义模式
function toggleCustomMode() {
    const el = document.getElementById('customMode');
    el.classList.toggle('show');
    if (el.classList.contains('show')) {
        el.scrollIntoView({ behavior: 'smooth' });
    }
}

// 切换设置面板
function toggleSettings() {
    document.getElementById('settingsPanel').classList.toggle('show');
}

// ==================== 快速场景库 ====================
function quickScenario(scenario) {
    let result = { cards: [], warning: null, fullscreen: false };
    
    // ========== 翻前场景 ==========
    
    if (scenario === 'preflop-aa-raise') {
        result.cards = [{
            type: 'success',
            title: '🚀 【行动】3-bet 到 3-4x 对手加注额',
            content: `【AA/KK面对加注】

✅ 标准打法：
• 对手加注3BB → 你3-bet到9-12BB
• 对手加注2.5BB → 你3-bet到8-10BB
• 有跟注者 → 每多1人+1BB

⚠️ 如果被4-bet：
• AA → 直接5-bet全下
• KK → 评估对手范围（紧凶可能弃牌）

🚨【绝不要做】
❌ 只跟注（慢打AA/KK = 送钱）
❌ 3-bet太小（给对手正确赔率）

💡 深筹码(200BB+)：
• AA可以考虑只跟注设陷阱
• 但必须有翻后技术支持`
        }];
    }
    
    else if (scenario === 'preflop-aa-3bet') {
        result.cards = [{
            type: 'warning',
            title: '💥 【行动】根据牌力决定',
            content: `【我加注被3-bet】

✅ 你有AA/KK：
• 4-bet到2.2-2.5x对手3-bet额
• 或者直接全下（筹码<100BB）

✅ 你有QQ/JJ/AK：
• 对手是紧凶 → 跟注看翻牌
• 对手是松凶 → 可以4-bet
• 筹码<60BB → 可以全下

✅ 你有中等牌(TT/99/AQs)：
• 有位置 → 跟注
• 没位置 → 弃牌或跟注

🚨【绝不要做】
❌ 用边缘牌跟注3-bet（会很被动）
❌ 4-bet后对手5-bet你弃牌（浪费筹码）`
        }];
    }
    
    else if (scenario === 'preflop-btn-open') {
        result.cards = [{
            type: 'success',
            title: '🎰 【行动】加注 2.5BB 开池',
            content: `【BTN/CO位开池范围】

✅ BTN位(最松)：
• 所有对子 22+
• 所有A高 A2s+, A7o+
• 所有K高同花 K2s+, K9o+
• Q高同花 Q5s+, Q9o+
• J高同花 J7s+, JTo
• 连张同花 54s+
• 大约40%的手牌

✅ CO位(稍紧)：
• 所有对子 22+
• A高 A2s+, A9o+
• K高 K9s+, KTo+
• Q高 Q9s+, QJo
• J高 J9s+, JTo
• 大约25%的手牌

🚨【绝不要做】
❌ 只加注好牌（太紧被盲注偷光）
❌ 平跟入池（没有主动权）

💡 面对盲注3-bet：
• 看对手类型决定跟注/4-bet/弃牌`
        }];
    }
    
    else if (scenario === 'preflop-defend-bb') {
        result.cards = [{
            type: 'info',
            title: '🛡️ 【行动】根据位置和筹码决定',
            content: `【大盲防守策略】

✅ 面对BTN加注(最松防守)：
• 跟注：22+, A2s+, A5o+, K5s+, Q7s+, 大部分连张
• 3-bet：AA-TT, AK, AQs, 偶尔加入诈唬

✅ 面对CO加注：
• 跟注：33+, A7s+, A9o+, KTs+, QTs+
• 3-bet：AA-JJ, AK, AQ

✅ 面对EP加注(最紧)：
• 跟注：77+, ATs+, KQs
• 3-bet：AA-QQ, AK

🚨【绝不要做】
❌ 用垃圾牌防守（打不赢翻后）
❌ 面对4-bet还跟注弱牌

💡 关键原则：
• 位置不好就要牌力补偿
• 深筹码多跟注，浅筹码多3-bet`
        }];
    }
    
    else if (scenario === 'preflop-squeeze') {
        result.cards = [{
            type: 'warning',
            title: '🔥 【行动】评估是否Squeeze',
            content: `【Squeeze机会评估】

✅ 适合Squeeze的情况：
• 你有强牌 (JJ+, AQ+)
• 原加注者位置靠后（范围宽）
• 跟注者是弱玩家
• 你在有利位置

✅ Squeeze尺寸：
• 标准：原加注4x + 每个跟注者+1x
• 例如：加注3BB + 1个跟注 → Squeeze到15-18BB

❌ 不适合Squeeze：
• 原加注者位置靠前（范围紧）
• 跟注者是紧凶玩家
• 你筹码太浅(<40BB)
• 你的牌太弱

🚨【绝不要做】
❌ Squeeze太小（给对手赔率）
❌ Squeeze后面对4-bet不知道怎么打`
        }];
    }
    
    else if (scenario === 'preflop-marginal') {
        result.cards = [{
            type: 'warning',
            title: '😬 【行动】位置决定一切',
            content: `【边缘牌决策】

✅ KJo/QJo/KTo 这类牌：
• 前位(UTG/MP) → 弃牌
• CO位 → 可以加注开池
• BTN位 → 加注开池
• 盲注位面对加注 → 通常弃牌

✅ QTs/JTs/T9s 同花连张：
• 前位 → 弃牌或小额加注
• 后位 → 加注开池
• 面对加注 → 有位置可跟

✅ 小对子 22-66：
• 深筹码(>80BB) → 跟注看翻牌
• 浅筹码(<50BB) → 弃牌或加注

🚨【绝不要做】
❌ 在前位用边缘牌加注
❌ 没位置用边缘牌跟注3-bet
❌ 多人底池用边缘牌追

💡 核心原则：
边缘牌 = 需要位置优势 + 翻后技术`
        }];
    }
    
    else if (scenario === 'preflop-deep') {
        result.cards = [{
            type: 'info',
            title: '💰 【建议】深筹码策略调整',
            content: `【深筹码(150BB+)策略】

✅ 应该更紧的情况：
• 翻前3-bet/4-bet
• 用边缘牌对抗紧凶玩家
• 多人底池

✅ 可以更松的情况：
• 有位置的跟注
• 同花连张/小对子（暗示赔率好）
• 对抗弱玩家

✅ 牌力调整：
• AK相对贬值（不容易全下）
• 同花连张升值（可以赢大底池）
• 小对子升值（中暗三赢大底池）

🚨【绝不要做】
❌ 翻前轻易全下（除非AA）
❌ 忽视位置的价值
❌ 用顶对打大底池

💡 深筹码核心：
• 控制底池
• 保护下风时的损失
• 最大化上风时的收益`
        }];
    }
    
    // ========== 翻牌场景 ==========
    
    else if (scenario === 'flop-set-wet') {
        result.cards = [{
            type: 'success',
            title: '🎯 【行动】下注 2/3 - Pot 保护',
            content: `【暗三在湿润牌面】

✅ 必须下注保护：
• 牌面有同花听牌 → 下注2/3-pot
• 牌面有顺子听牌 → 下注2/3-pot
• 多人底池 → 下注更大

⚠️ 如果被加注：
• 牌面没完成听牌 → 全下
• 牌面很湿 → 跟注看转牌

🚨【绝不要做】
❌ 过牌慢打（给免费牌太危险）
❌ 小额下注（给对手正确赔率）

💡 转牌/河牌预警：
• 完成同花 + 对手大额下注 → 你要能弃暗三
• 记住：暗三不是无敌的`
        }];
    }
    
    else if (scenario === 'flop-toptwo') {
        result.cards = [{
            type: 'success',
            title: '✌️ 【行动】下注 1/2 - 2/3 pot',
            content: `【顶两对打法】

✅ 标准打法：
• 有位置 → 下注1/2-2/3 pot
• 没位置 → 下注1/2 pot

⚠️ 牌面湿润时：
• 下注更大(2/3 pot)保护
• 不要给听牌便宜的价格

🚨【危险警告】
• 转牌/河牌牌面配对 → 对手可能有葫芦
• 转牌/河牌完成听牌 → 重新评估

💡 记住：两对不是坚果
• 对手加注要认真考虑
• 河牌大额下注要能弃牌`
        }];
    }
    
    else if (scenario === 'flop-overpair') {
        result.cards = [{
            type: 'success',
            title: '👑 【行动】持续下注 1/2 pot',
            content: `【超对（翻牌后AA/KK）】

✅ 标准打法：
• 干燥牌面(如K72) → 下注1/3 pot
• 湿润牌面(如T98) → 下注1/2-2/3 pot

⚠️ 被过牌加注：
• 干燥牌面 → 对手可能有暗三，谨慎
• 湿润牌面 → 可能是听牌加注，跟注

🚨【不要高估超对】
• 翻牌有3张连牌 → 对手可能有顺子
• 翻牌有同花面 → 对手可能有同花听牌
• 多人底池 → 超对价值下降

💡 核心原则：
超对 = 好牌但不是坚果
底池越大，超对相对价值越低`
        }];
    }
    
    else if (scenario === 'flop-flushdraw') {
        result.cards = [{
            type: 'info',
            title: '♠️ 【行动】半诈唬或过牌',
            content: `【同花听牌策略】

✅ 你有位置 + 对手过牌：
• 下注1/2 pot半诈唬
• 对手弃牌你赢
• 对手跟注你有36%胜率

✅ 你没位置：
• 可以过牌（免费看牌）
• 也可以领先下注半诈唬

✅ 对手下注：
• 下注<1/2 pot → 跟注（赔率够）
• 下注>2/3 pot → 考虑弃牌

📊 同花听牌胜率：
• 翻牌到河牌：约35%
• 转牌到河牌：约18%

🚨【绝不要做】
❌ 不计算赔率就跟大额下注
❌ 在多人底池诈唬加注`
        }];
    }
    
    else if (scenario === 'flop-oesd') {
        result.cards = [{
            type: 'info',
            title: '🔢 【行动】计算赔率决定',
            content: `【两头顺听牌策略】

✅ 8个outs，胜率：
• 翻牌到河牌：约31%
• 转牌到河牌：约16%

✅ 对手过牌：
• 可以下注半诈唬
• 或者过牌免费看牌

✅ 对手下注：
• 下注<1/2 pot → 跟注
• 下注>pot → 弃牌

🚨【注意事项】
• 你的顺子可能不是坚果
• 牌面有同花 → 你的顺子可能输给同花
• 牌面有对子 → 小心葫芦

💡 两头顺 vs 卡顺：
• 两头顺(8outs) → 值得追
• 卡顺(4outs) → 一般不值得追`
        }];
    }
    
    else if (scenario === 'flop-cbet-miss') {
        result.cards = [{
            type: 'warning',
            title: '🎭 【行动】评估是否C-bet',
            content: `【翻牌没中，是否持续下注】

✅ 应该C-bet的情况：
• 干燥牌面(A72, K83)
• 对手只有1人
• 牌面对你范围有利

✅ 不应该C-bet的情况：
• 湿润牌面(JT9, 876)
• 多人底池
• 对手是跟注站

✅ C-bet尺寸：
• 干燥牌面 → 1/3 pot
• 湿润牌面 → 1/2 pot或放弃

🚨【被加注怎么办】
• 你有后门听牌 → 可以跟注
• 你完全没有牌力 → 弃牌

💡 核心原则：
不是每个翻牌都要C-bet
放弃也是一种选择`
        }];
    }
    
    else if (scenario === 'flop-checkraise') {
        result.cards = [{
            type: 'critical',
            title: '🚨 【行动】谨慎评估',
            content: `【被过牌加注】

✅ 对手可能有：
• 暗三/两对（价值加注）
• 强听牌（半诈唬加注）
• 纯诈唬（较少）

✅ 你应该：
• 你有顶对好踢脚 → 跟注
• 你有超对 → 跟注
• 你有暗三 → 可以再加注
• 你只有中对/弱顶对 → 弃牌

🚨【危险信号】
• 对手是被动玩家过牌加注 = 100%有货
• 对手在干燥牌面过牌加注 = 很可能暗三

💡 记住：
翻牌过牌加注通常是强牌
不要用边缘牌抗`
        }];
    }
    
    else if (scenario === 'flop-donk') {
        result.cards = [{
            type: 'warning',
            title: '❓ 【行动】根据对手类型决定',
            content: `【对手领先下注(Donk Bet)】

✅ 对手可能是：
• 弱玩家 → 通常有成牌但不知道怎么打
• 强玩家 → 可能是试探或价值

✅ 你有强牌(顶对+)：
• 加注获取价值
• 或跟注（如果对手会弃牌）

✅ 你有中等牌：
• 跟注看看
• 转牌对手继续下注 → 重新评估

✅ 你没中：
• 弃牌（不要浮打）
• 除非你有位置+后门听牌

💡 核心原则：
Donk bet通常表示中等牌力
对手很少用Donk来诈唬`
        }];
    }
    
    // ========== 转牌场景 ==========
    
    else if (scenario === 'turn-strong-safe') {
        result.cards = [{
            type: 'success',
            title: '💪 【行动】下注 2/3 pot 取价值',
            content: `【强牌+安全牌面】

✅ 标准打法：
• 下注2/3 pot
• 建立大底池

⚠️ 如果对手加注：
• 你有坚果/暗三 → 全下
• 你有两对 → 跟注（但河牌谨慎）

💡 河牌计划：
• 对手跟注 → 河牌继续下注2/3-pot
• 对手过牌 → 河牌下注pot取价值

🚨【不要做】
❌ 过牌慢打（错过价值）
❌ 小额下注（没有最大化）`
        }];
    }
    
    else if (scenario === 'turn-strong-danger') {
        result.cards = [{
            type: 'warning',
            title: '😰 【行动】下注 1/2 pot 控制底池',
            content: `【强牌但牌面变危险】

✅ 转牌出现第3张同花/顺子连牌：
• 下注1/2 pot（控制底池）
• 不要过度投入

⚠️ 如果对手加注：
• 小加注 → 跟注
• 大加注(≥pot) → 考虑弃牌

🚨【河牌预警】
• 完成同花/顺子 + 对手大额下注 → 弃牌
• 你的两对/暗三可能已经输了

💡 核心原则：
转牌投入越少，河牌越好弃
牌面危险就要控制底池`
        }];
    }
    
    else if (scenario === 'turn-raised') {
        result.fullscreen = true;
        result.warning = {
            title: '你下注被加注了！',
            text: '转牌被加注通常代表强牌\n请冷静评估',
            action: '【评估】对手范围',
            dont: '❌ 不要情绪化跟注\n❌ 不要不思考就弃牌'
        };
        result.cards = [{
            type: 'critical',
            title: '💀 【行动】根据牌力决定',
            content: `【转牌被加注】

✅ 你有坚果/暗三：
• 全下！

✅ 你有两对：
• 对手是紧凶 → 弃牌（他范围很强）
• 对手是松凶 → 可以跟注
• 对手是被动鱼 → 100%弃牌

✅ 你有超对/顶对：
• 通常弃牌
• 一对牌不值得打大底池

🚨【关键判断】
• 被动玩家转牌加注 = 100%有货
• 紧凶玩家转牌加注 = 很强的范围
• 松凶玩家转牌加注 = 范围较宽，可以抓

💡 记住你标记的错误：
"转牌顶对/两对跟注大额加注"
今天不要犯这个错！`
        }];
    }
    
    else if (scenario === 'turn-big-bet') {
        result.cards = [{
            type: 'critical',
            title: '💣 【行动】谨慎！通常应该弃牌',
            content: `【对手转牌突然大额下注】

✅ 大额下注(≥2/3 pot)通常意味着：
• 强成牌想取价值
• 很少是诈唬

✅ 你应该：
• 你有坚果 → 加注
• 你有暗三 → 跟注（但河牌警惕）
• 你有两对 → 谨慎跟注
• 你有一对 → 弃牌

🚨【被动鱼大额下注】
• 100%有牌，直接弃（除非你有坚果）

💡 转牌大额下注的本质：
对手想建立大底池
问自己：我的牌够强吗？`
        }];
    }
    
    else if (scenario === 'turn-draw-check') {
        result.cards = [{
            type: 'success',
            title: '🎯 【行动】下注 1/2 pot 半诈唬',
            content: `【听牌+对手过牌】

✅ 标准打法：
• 下注1/2 pot半诈唬
• 对手弃牌 → 你赢
• 对手跟注 → 你还有胜率

✅ 半诈唬的好处：
• 立即赢底池的可能
• 建立诈唬形象
• 中牌时赢更多

🚨【如果对手加注】
• 计算赔率
• 赔率够 → 跟注
• 赔率不够 → 弃牌

💡 同花听牌胜率约18%（转到河）
需要至少3:1赔率才能盈利跟注`
        }];
    }
    
    else if (scenario === 'turn-draw-bet') {
        result.cards = [{
            type: 'warning',
            title: '📊 【行动】计算赔率决定！',
            content: `【听牌+对手下注】

📊 赔率计算：
跟注金额 ÷ (底池+下注+跟注) = 需要胜率

✅ 同花听牌(9outs)≈18%：
• 对手下注1/3 pot → 跟（需要20%）
• 对手下注1/2 pot → 跟（需要25%）
• 对手下注pot → 弃（需要33%）

✅ 两头顺(8outs)≈16%：
• 对手下注1/3 pot → 勉强跟
• 对手下注≥1/2 pot → 弃

🚨【绝不要做】
❌ "感觉会中" → 情绪化跟注
❌ 不计算就跟大额下注

💡 职业玩家永远用正确赔率决策`
        }];
    }
    
    else if (scenario === 'turn-onepair') {
        result.cards = [{
            type: 'warning',
            title: '1️⃣ 【行动】控制底池',
            content: `【转牌只有一对】

✅ 牌面安全 + 对手过牌：
• 下注1/3-1/2 pot薄价值

✅ 牌面危险 + 对手过牌：
• 过牌（控制底池）

✅ 对手下注：
• 小额下注 → 跟注
• 大额下注 → 弃牌

🚨【核心原则】
• 一对牌 ≠ 好牌
• 不要用一对打大底池
• 转牌投入越少，河牌越好弃

💡 常见错误：
用顶对跟注大额下注 → 输大底池
应该在转牌就控制损失`
        }];
    }
    
    // ========== 河牌场景 ==========
    
    else if (scenario === 'river-completed-bigbet') {
        result.fullscreen = true;
        result.warning = {
            title: '停！你的强牌可能已经输了！',
            text: '河牌完成了同花/顺子\n对手大额下注 = 99%有坚果',
            action: '【弃牌】是正确答案',
            dont: '❌ 不要跟注\n❌ 不要Hero Call\n❌ 不要想"他可能诈唬"'
        };
        result.cards = [{
            type: 'critical',
            title: '🚨 【行动】弃牌！',
            content: `【河牌完成听牌+对手大额下注】

💀 情况分析：
• 河牌完成了同花/顺子
• 对手大额下注
• 你的暗三/两对已经不是好牌了

🚨 对手99%有坚果：
• 被动玩家河牌大额下注 = 100%有牌
• 你跟注只会输更多

【职业vs业余分水岭】
• 业余：舍不得弃暗三 → 输50-200BB
• 职业：完成听牌秒弃 → 省50-200BB

💡 今天不要犯这个错！
河牌完成听牌 + 对手大额下注 = 弃牌`
        }];
    }
    
    else if (scenario === 'river-allin') {
        result.fullscreen = true;
        result.warning = {
            title: '对手河牌全下！',
            text: '河牌全下通常是极化范围\n要么坚果，要么诈唬',
            action: '【冷静分析】',
            dont: '❌ 不要冲动跟注\n❌ 不要情绪化决策'
        };
        result.cards = [{
            type: 'critical',
            title: '☠️ 【行动】评估是否跟注',
            content: `【对手河牌全下】

✅ 你应该跟注的情况：
• 你有坚果 → 100%跟
• 你有第二坚果 + 对手是松凶 → 可能跟
• 赔率非常好(>3:1) + 你有Bluff Catcher

❌ 你应该弃牌的情况：
• 牌面完成听牌 + 你不是坚果
• 对手是被动玩家
• 你只有一对

🚨【被动玩家全下】
• 100%有牌，直接弃

💡 关键问题：
• 对手的诈唬频率是多少？
• 你需要多少胜率才能跟？
• 对手会用什么诈唬？`
        }];
    }
    
    else if (scenario === 'river-fish-bigbet') {
        result.fullscreen = true;
        result.warning = {
            title: '被动鱼大额下注！',
            text: '被动鱼大额下注 = 100%有强牌\n他不会诈唬',
            action: '【弃牌】除非你有坚果',
            dont: '❌ 不要跟注\n❌ 不要想"他可能在诈唬"'
        };
        result.cards = [{
            type: 'critical',
            title: '🐟 【行动】弃牌！',
            content: `【被动鱼大额下注】

💀 关键信息：
• 对手是被动鱼
• 被动玩家大额下注 = 100%有牌
• 他们不懂诈唬，只在有货时下注

🚨 你的强牌打不赢：
• 暗三 → 弃牌
• 两对 → 弃牌
• 超对 → 弃牌
• 顶对 → 弃牌

✅ 只有坚果才能跟

💡 记住你标记的错误：
"被动鱼大额下注还跟注"
这是最容易犯的错误！`
        }];
    }
    
    else if (scenario === 'river-nuts-check') {
        result.cards = [{
            type: 'success',
            title: '🏆 【行动】下注 Pot 取最大价值',
            content: `【坚果+对手过牌】

✅ 标准打法：
• 下注 3/4 pot 到 pot
• 最大化你的价值

💡 尺度选择：
• 对手是跟注站 → 下注pot
• 对手是紧凶 → 下注2/3 pot
• 如果对手加注 → 全下！

🚨【绝不要做】
❌ 过牌（浪费坚果）
❌ 小额下注（没有最大化价值）

你有坚果就要拿最大价值！`
        }];
    }
    
    else if (scenario === 'river-nuts-bet') {
        result.cards = [{
            type: 'success',
            title: '💎 【行动】加注或全下',
            content: `【坚果+对手下注】

✅ 标准打法：
• 对手小额下注 → 加注3x
• 对手大额下注 → 全下！

💡 为什么要加注：
• 你有坚果，要拿最大价值
• 对手已经下注，说明他有牌
• 不加注 = 浪费价值

🚨【绝不要做】
❌ 只跟注（浪费坚果价值）
❌ 害怕对手弃牌（他已经下注了）`
        }];
    }
    
    else if (scenario === 'river-strong-check') {
        result.cards = [{
            type: 'success',
            title: '💪 【行动】价值下注 1/2 - 2/3 pot',
            content: `【强牌（非坚果）+对手过牌】

✅ 牌面安全：
• 下注2/3 pot取价值

✅ 牌面危险（有完成的听牌）：
• 下注1/2 pot薄价值
• 或者过牌控制

⚠️ 如果对手加注：
• 认真评估是否被超越
• 牌面完成听牌 → 可能弃牌
• 牌面安全 → 跟注

💡 核心原则：
强牌不是坚果
河牌被加注要能弃掉`
        }];
    }
    
    else if (scenario === 'river-bluffcatch') {
        result.cards = [{
            type: 'warning',
            title: '🎭 【行动】分析对手范围决定',
            content: `【对手下注，你只有Bluff Catcher】

✅ 决定因素：
• 对手类型（松凶更可能诈唬）
• 牌面结构（听牌都没中 → 对手可能诈唬）
• 下注尺度（超额下注 → 可能诈唬）

✅ 跟注的条件：
• 对手是松凶LAG
• 牌面听牌都没中
• 你的赔率足够好

❌ 弃牌的条件：
• 对手是被动玩家
• 对手是紧凶TAG
• 牌面完成了听牌

💡 职业技巧：
算出对手需要多少诈唬频率你才能盈利
然后判断对手是否会诈唬那么多`
        }];
    }
    
    else if (scenario === 'river-missed-check') {
        result.cards = [{
            type: 'warning',
            title: '💨 【行动】诈唬或放弃',
            content: `【听牌没中+对手过牌】

✅ 可以诈唬的情况：
• 对手范围很弱（之前很被动）
• 你的范围有很多完成的听牌
• 对手是紧凶容易弃牌

✅ 诈唬尺度：
• 下注2/3-pot
• 要看起来像价值下注

❌ 不应该诈唬的情况：
• 对手是跟注站
• 你之前表现很弱
• 对手可能陷阱

💡 核心原则：
诈唬要有故事
问自己：如果我真的有坚果，我会这样打吗？`
        }];
    }
    
    else if (scenario === 'river-missed-bet') {
        result.cards = [{
            type: 'critical',
            title: '🙈 【行动】直接弃牌',
            content: `【听牌没中+对手下注】

❌ 标准打法：弃牌

🚨【绝不要做】
❌ 跟注（你什么都没有）
❌ 诈唬加注（风险太大）

💡 为什么直接弃牌：
• 你没有成牌
• 你没有摊牌价值
• 跟注 = 100%输钱
• 诈唬加注 = 高风险低收益

接受现实，保留筹码打下一手`
        }];
    }
    
    displayResult(result);
}

// ==================== 自定义分析（保持原有功能）====================
function analyze() {
    let pot = parseFloat(document.getElementById('potSize').value) || 50;
    if (pot <= 0) pot = 50;
    
    const bet = {
        third: Math.round(pot * 0.33),
        half: Math.round(pot * 0.5),
        twoThird: Math.round(pot * 0.67),
        pot: pot
    };
    
    let result = { cards: [], warning: null, fullscreen: false, personal: [] };
    const { position, street, hand, action, villain, danger } = state;
    
    const mistakes = {
        m1: document.getElementById('m1')?.checked,
        m2: document.getElementById('m2')?.checked,
        m3: document.getElementById('m3')?.checked,
        m4: document.getElementById('m4')?.checked,
        m5: document.getElementById('m5')?.checked
    };
    
    // 核心场景：全屏警告
    if (street === 'river' && danger === 'completed' && 
        (action === 'bet-big' || action === 'all-in') &&
        (hand === 'set' || hand === 'top-two' || hand === 'overpair' || hand === 'top-pair')) {
        
        result.fullscreen = true;
        result.warning = {
            title: `停！你的${getHandName(hand)}已经输了！`,
            text: '河牌完成了同花/顺子\n对手大额下注 = 99%有坚果',
            action: '【弃牌】是正确答案',
            dont: '❌ 不要跟注\n❌ 不要Hero Call\n❌ 不要想"他可能诈唬"'
        };
        
        if (mistakes.m2) result.personal.push('🚨 你标记过：河牌完成听牌舍不得弃暗三');
        
        result.cards.push({
            type: 'critical',
            title: '🚨 【行动】弃牌！',
            content: `【情况分析】
• 河牌完成了听牌（同花/顺子）
• 对手大额下注
• 你的${getHandName(hand)}已经被超越

💀【对手99%有坚果】
${villain === 'fish' ? '• 被动鱼大额下注 = 100%有牌！' : '• 河牌大额下注通常是强牌'}

【职业玩家的选择】弃牌！省下50-200BB`
        });
    }
    
    else if (villain === 'fish' && (action === 'bet-big' || action === 'all-in') && hand !== 'nuts') {
        result.fullscreen = true;
        result.warning = {
            title: '被动鱼大额下注！',
            text: '被动鱼大额下注 = 100%有强牌\n他不会诈唬',
            action: '【弃牌】除非你有坚果',
            dont: '❌ 不要跟注\n❌ 不要想"他可能在诈唬"'
        };
        
        if (mistakes.m3) result.personal.push('🚨 你标记过：被动鱼大额下注还跟注');
        
        result.cards.push({
            type: 'critical',
            title: '🚨 【行动】弃牌！',
            content: `【关键信息】
• 对手是被动鱼
• 被动玩家大额下注 = 100%有牌
• 他们不懂诈唬，只在有货时下注

💀【你的${getHandName(hand)}打不赢】
弃牌是唯一正确选择`
        });
    }
    
    // 转牌圈逻辑
    else if (street === 'turn') {
        if (hand === 'nuts') {
            result.cards.push({
                type: 'success',
                title: `🎯 【行动】下注 ${bet.twoThird}BB（2/3底池）`,
                content: `【原因】你有坚果，价值最大化\n\n【如果对手加注】直接全下！\n\n🚨【绝不要做】\n❌ 过牌慢打\n❌ 小额下注`
            });
        }
        else if (hand === 'set') {
            if (danger === 'dangerous' || danger === 'completed') {
                result.cards.push({
                    type: 'warning',
                    title: `🎯 【行动】下注 ${bet.twoThird}BB（2/3底池·保护）`,
                    content: `【原因】暗三但牌面危险，必须保护\n\n【如果对手加注】谨慎评估\n\n💡【河牌预警】完成听牌 + 对手大额下注 → 弃暗三！`
                });
                if (mistakes.m4) result.personal.push('🚨 你标记过：转牌给听牌免费抽牌机会');
            } else {
                result.cards.push({
                    type: 'success',
                    title: `🎯 【行动】下注 ${bet.twoThird}BB（2/3底池）`,
                    content: `【原因】牌面安全，暗三价值最大化\n\n【如果对手加注】全下！`
                });
            }
        }
        else if (hand === 'top-two') {
            if (danger === 'dangerous' || danger === 'completed') {
                if (action === 'bet-big' || action === 'all-in') {
                    result.cards.push({
                        type: 'critical',
                        title: `🚨 【行动】弃牌或小额跟注`,
                        content: `【情况】顶两对 + 牌面危险 + 对手大额下注\n\n【决策】\n• 对手是紧凶TAG → 弃牌\n• 对手是松凶LAG → 可以跟注\n• 对手是被动鱼 → 绝对弃牌！`
                    });
                    if (mistakes.m1) result.personal.push('🚨 你标记过：转牌顶对/两对跟注大额加注');
                } else {
                    result.cards.push({
                        type: 'warning',
                        title: `🎯 【行动】下注 ${bet.half}BB（1/2底池·控制）`,
                        content: `【原因】牌面危险，控制底池\n\n【如果对手加注】\n• 小加注 → 跟注\n• 大加注(≥pot) → 弃牌！\n\n💡【河牌预警】完成听牌 + 对手下注 → 弃掉两对！`
                    });
                }
            } else {
                result.cards.push({
                    type: 'success',
                    title: `🎯 【行动】下注 ${bet.twoThird}BB（2/3底池）`,
                    content: `【原因】牌面安全，顶两对价值下注\n\n【如果对手加注】谨慎评估（可能有暗三）`
                });
            }
        }
        else if (hand === 'overpair' || hand === 'top-pair') {
            if (danger === 'dangerous' || danger === 'completed') {
                if (action === 'bet-big' || action === 'all-in') {
                    result.cards.push({
                        type: 'critical',
                        title: '🚨 【行动】弃牌',
                        content: `【情况】一对 + 牌面危险 + 对手大额下注\n\n💀 弃牌是正确选择`
                    });
                } else {
                    result.cards.push({
                        type: 'warning',
                        title: `🎯 【行动】过牌 或 下注 ${bet.third}BB`,
                        content: `【原因】一对牌 + 牌面危险 = 控制底池\n\n【如果对手下注】\n• 小额 → 可跟注\n• 大额 → 弃牌`
                    });
                }
            } else {
                result.cards.push({
                    type: 'success',
                    title: `🎯 【行动】下注 ${bet.half}BB（1/2底池）`,
                    content: `【原因】牌面安全，保护+薄价值\n\n【如果对手加注≥pot】弃牌`
                });
            }
        }
        else if (hand === 'nuts-draw') {
            if (action === 'check') {
                result.cards.push({
                    type: 'success',
                    title: `🎯 【行动】下注 ${bet.half}BB（半诈唬）`,
                    content: `【原因】对手过牌 → 半诈唬\n• 对手弃牌 → 你赢\n• 对手跟注 → 你还有约36%胜率`
                });
            } else if (action === 'bet-big' || action === 'all-in') {
                result.cards.push({
                    type: 'critical',
                    title: '🚨 【行动】弃牌（赔率不够）',
                    content: `【计算】对手下注pot → 赔率约33%，你胜率约18%\n\n🚨 胜率 < 赔率 → 弃牌`
                });
                if (mistakes.m5) result.personal.push('🚨 你标记过：不计算赔率就追听牌');
            } else {
                result.cards.push({
                    type: 'warning',
                    title: '🎯 【行动】计算赔率决定',
                    content: `【同花听牌】约18%胜率\n\n跟注金额 ÷ 总底池 = 需要胜率\n• 胜率 > 需要胜率 → 跟注\n• 胜率 < 需要胜率 → 弃牌`
                });
            }
        }
        else if (hand === 'draw') {
            result.cards.push({
                type: 'warning',
                title: '🎯 【行动】计算赔率决定',
                content: `【听牌胜率】\n• 两头顺(8outs)≈16%\n• 卡顺(4outs)≈8%\n\n【决策】\n• 对手过牌 → 免费看河牌\n• 对手小额下注 → 计算赔率\n• 对手大额下注 → 弃牌`
            });
        }
        else if (hand === 'nothing' || hand === 'mid-pair') {
            result.cards.push({
                type: 'critical',
                title: '🚨 【行动】过牌-弃牌',
                content: `【原因】牌太弱，没有价值\n不要诈唬（转牌诈唬成功率低）\n\n💡 及时止损 = 职业玩家标志`
            });
        }
    }
    
    // 河牌圈逻辑
    else if (street === 'river') {
        if (hand === 'nuts') {
            if (action === 'check') {
                result.cards.push({
                    type: 'success',
                    title: `🎯 【行动】下注 ${bet.pot}BB（Pot）`,
                    content: `【原因】你有坚果，最大化价值\n\n【对手加注】全下！`
                });
            } else {
                result.cards.push({
                    type: 'success',
                    title: '🎯 【行动】加注或全下',
                    content: `【原因】你有坚果，对手下注了\n\n• 对手小额下注 → 加注3x\n• 对手大额下注 → 全下`
                });
            }
        }
        else if (hand === 'set' || hand === 'top-two') {
            if (danger === 'safe' || danger === 'normal') {
                if (action === 'check') {
                    result.cards.push({
                        type: 'success',
                        title: `🎯 【行动】下注 ${bet.twoThird}BB（价值）`,
                        content: `【原因】牌面安全，对手过牌，价值下注\n\n【对手加注】谨慎（可能慢打坚果）`
                    });
                } else {
                    result.cards.push({
                        type: 'warning',
                        title: '🎯 【行动】跟注或弃牌',
                        content: `【决策】\n• 对手小额下注 → 跟注\n• 对手大额下注 → 评估是否被超越`
                    });
                }
            } else {
                if (action === 'check') {
                    result.cards.push({
                        type: 'warning',
                        title: `🎯 【行动】下注 ${bet.half}BB（薄价值/探底）`,
                        content: `【原因】牌面危险，但对手过牌显示弱势\n\n【如果对手加注】考虑弃牌`
                    });
                } else if (action === 'bet-small') {
                    result.cards.push({
                        type: 'warning',
                        title: '🎯 【行动】跟注',
                        content: `【原因】对手小额下注，可以跟注抓诈唬\n\n【注意】如果对手后续大额下注 → 考虑弃牌`
                    });
                } else {
                    result.cards.push({
                        type: 'critical',
                        title: '🚨 【行动】谨慎弃牌',
                        content: `【情况】牌面危险/完成听牌 + 对手大额下注\n\n你的${getHandName(hand)}可能已经输了\n\n💡 保守弃牌是正确选择`
                    });
                }
            }
        }
        else if (hand === 'overpair' || hand === 'top-pair' || hand === 'mid-pair') {
            if (action === 'bet-big' || action === 'all-in') {
                result.cards.push({
                    type: 'critical',
                    title: '🚨 【行动】弃牌',
                    content: `【原因】河牌对手大额下注，你只有一对，打不赢任何价值牌\n\n❌ 不要Hero Call`
                });
            } else if (action === 'check') {
                if (danger === 'safe' || danger === 'normal') {
                    result.cards.push({
                        type: 'success',
                        title: `🎯 【行动】下注 ${bet.half}BB（薄价值）`,
                        content: `【原因】对手过牌，牌面安全\n\n【对手加注】弃牌`
                    });
                } else {
                    result.cards.push({
                        type: 'warning',
                        title: '🎯 【行动】过牌（摊牌）',
                        content: `【原因】牌面危险，不要下注\n\n过牌看摊牌`
                    });
                }
            } else {
                result.cards.push({
                    type: 'warning',
                    title: '🎯 【行动】弃牌或跟注',
                    content: `【决策】\n• 顶对好踢脚 + 牌面安全 → 可跟\n• 其他情况 → 弃牌`
                });
            }
        }
        else if (hand === 'nuts-draw' || hand === 'draw' || hand === 'nothing') {
            if (action === 'check') {
                result.cards.push({
                    type: 'warning',
                    title: `🎯 【行动】诈唬 ${bet.twoThird}BB 或 放弃`,
                    content: `【选项】\n• 对手范围弱 → 诈唬下注${bet.twoThird}BB\n• 对手可能陷阱 → 过牌放弃`
                });
            } else {
                result.cards.push({
                    type: 'critical',
                    title: '🚨 【行动】弃牌',
                    content: `【原因】听牌没中 + 对手下注\n\n直接弃牌，不要诈唬加注`
                });
            }
        }
    }
    
    // 翻牌圈逻辑
    else if (street === 'flop') {
        if (hand === 'nuts' || hand === 'set') {
            result.cards.push({
                type: 'success',
                title: `🎯 【行动】下注 ${bet.half}BB - ${bet.twoThird}BB`,
                content: `【原因】强牌，建立底池\n\n• 干燥牌面 → ${bet.half}BB\n• 湿润牌面 → ${bet.twoThird}BB`
            });
        } else if (hand === 'top-two' || hand === 'overpair' || hand === 'top-pair') {
            result.cards.push({
                type: 'success',
                title: `🎯 【行动】下注 ${bet.half}BB`,
                content: `【原因】持续下注，保护+价值\n\n【对手加注】评估决定`
            });
        } else if (hand === 'nuts-draw' || hand === 'draw') {
            result.cards.push({
                type: 'success',
                title: `🎯 【行动】下注 ${bet.half}BB 或 过牌`,
                content: `【选项】\n• 下注${bet.half}BB半诈唬\n• 过牌免费看转牌\n\n【对手下注】计算赔率决定`
            });
        } else {
            result.cards.push({
                type: 'warning',
                title: '🎯 【行动】过牌',
                content: `【原因】弱牌，不要投入\n\n对手下注 → 弃牌`
            });
        }
    }
    
    if (street === 'turn') {
        document.getElementById('continueMode').style.display = 'block';
    }
    
    displayResult(result);
}

function getHandName(hand) {
    const names = {
        'nuts': '坚果', 'set': '暗三', 'top-two': '顶两对',
        'overpair': '超对', 'top-pair': '顶对', 'mid-pair': '中对',
        'nuts-draw': 'Nuts听牌', 'draw': '听牌', 'nothing': '空气'
    };
    return names[hand] || hand;
}

function displayResult(result) {
    if (result.fullscreen && result.warning) {
        document.getElementById('warningTitle').textContent = result.warning.title;
        document.getElementById('warningText').textContent = result.warning.text;
        document.getElementById('warningAction').textContent = result.warning.action;
        document.getElementById('warningDont').textContent = result.warning.dont;
        document.getElementById('fullscreenWarning').classList.add('show');
    }
    
    const personalEl = document.getElementById('personalWarning');
    if (result.personal && result.personal.length > 0) {
        personalEl.innerHTML = result.personal.map(p => 
            `<div class="personal-warning">
                <div class="personal-warning-title">⚠️ 你的常犯错误</div>
                <div class="personal-warning-text">${p}</div>
            </div>`
        ).join('');
    } else {
        personalEl.innerHTML = '';
    }
    
    const container = document.getElementById('resultContainer');
    container.innerHTML = result.cards.map(card => 
        `<div class="result-card result-${card.type}">
            <div class="result-title">${card.title}</div>
            <div class="result-content">${card.content}</div>
        </div>`
    ).join('');
    
    document.getElementById('resultSection').classList.add('show');
    document.getElementById('resultSection').scrollIntoView({ behavior: 'smooth' });
}

function confirmFold() {
    document.getElementById('fullscreenWarning').classList.remove('show');
}

function closeWarning() {
    document.getElementById('fullscreenWarning').classList.remove('show');
}

function resetAll() {
    document.getElementById('potSize').value = '50';
    document.querySelectorAll('.btn').forEach(btn => btn.classList.remove('active'));
    document.querySelector('[data-position="ip"]')?.classList.add('active');
    document.querySelector('[data-street="turn"]')?.classList.add('active');
    document.querySelector('[data-hand="top-two"]')?.classList.add('active');
    document.querySelector('[data-action="check"]')?.classList.add('active');
    document.querySelector('[data-villain="unknown"]')?.classList.add('active');
    document.querySelector('[data-danger="dangerous"]')?.classList.add('active');
    
    document.getElementById('resultSection').classList.remove('show');
    document.getElementById('customMode').classList.remove('show');
    document.getElementById('continueMode').style.display = 'none';
    document.getElementById('personalWarning').innerHTML = '';
    document.getElementById('resultContainer').innerHTML = '';
    document.getElementById('fullscreenWarning').classList.remove('show');
    
    // 重置街道选项卡
    document.querySelectorAll('.street-tab').forEach(t => t.classList.remove('active'));
    document.querySelectorAll('.scenario-panel').forEach(p => p.classList.remove('active'));
    document.querySelector('.street-tab').classList.add('active');
    document.getElementById('panel-preflop').classList.add('active');
    
    state = {
        position: 'ip', street: 'turn', hand: 'top-two',
        action: 'check', villain: 'unknown', danger: 'dangerous'
    };
    
    window.scrollTo({ top: 0, behavior: 'smooth' });
}
